...
export async function getElectronics() {
  ...
  try {
    const results = await Promise.all(
      categories.map((cat) =>
        axios.get(`${baseUrl}/products/category/${cat}?limit=20`).then((r) => r.data.products)
      )
    )
    cachedProducts = results.flat().slice(0, 36)
...
export async function getCategory(slug) {
  ...
  if (!meta) return []
  try {
    const res = await axios.get(`${baseUrl}/products/category/${meta.api}?limit=30`)
